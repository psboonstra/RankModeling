...
# can be submitted at once to the scheduler)
# (ii) sum_{i=1}^{jobs_per_scenario} n_sim_i = total desired sims per scenario
jobs_per_scenario = ifelse(my_computer, 27, 27);
...
array_id_offset = 0;
# Should the sim_id labels be randomly permuted across array_ids? (note: the
# first 36 array_ids are never permuted. All array_ids beyond 36 are permuted)
...
# This is the actual call to the function 'simulator'
assign(paste0("sim",array_id_offset + array_id),do.call("simulator",args = curr_args));


# Save the results: an R binary file of the entire output and two csv files. 
# The destination folder is called out
